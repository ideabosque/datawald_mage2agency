...
class Mage2Agency(Agency):
    ...
    def update_mage2_order(self, increment_id, transaction, type, warehouse=None):
        ...
        for ns_item in items:
            if (
                isinstance(ns_item.get("coa_file_urls", None), list)
                and len(ns_item.get("coa_file_urls", [])) > 0
            ):
        ...
        if warehouse is not None:
            order_items = self.mage2OrderConnector.get_order_items(
                order.get("entity_id")
            )
            for ns_item in items:
                for order_item in order_items:
                    if order_item.get(
                        "parent_item_id", None
                    ) is None and order_item.get("sku") == ns_item.get("sku"):
                        product_options = (
                            json.loads(order_item.get("product_options"))
                            if order_item.get("product_options", None) is not None
                            else None
                        )
                        if (
                            product_options
                            and product_options.get("info_buyRequest", {}).get(
                                "warehouse"
                            )
                            == warehouse
                        ):
        ...
        if transaction["data"].get("sales_rep_name", None):
            sales_rep_name_comment = "Sales Rep: {sales_rep_name}".format(
                sales_rep_name=transaction["data"].get("sales_rep_name")
            )
            self.mage2OrderConnector.insert_order_comment(
                order_id=order["entity_id"],
                comment=sales_rep_name_comment,
                status=order.get("status"),
            )

        order_ns_status = transaction["data"].get("status")
        if (
            self.mage2OrderConnector.can_invoice_order(order)
            and order_ns_status == "Billed"
        ):
            can_invoice = True
            if len(
                api_item_ids
            ) > 0 and not self.mage2OrderConnector.can_invoice_order_items(
                order, api_item_ids
            ):
                can_invoice = False
            if can_invoice:
        ...
        if (
            self.mage2OrderConnector.can_ship_order(order)
            and transaction["data"].get("fulfill_ship_status") == "_shipped"
            and order_ns_status == "Billed"
        ):
            can_ship = True
            if len(
                api_item_ids
            ) > 0 and not self.mage2OrderConnector.can_ship_order_items(
                order, api_item_ids
            ):
                can_ship = False
        ...
        if transformed_status is not None:
            ...
            if warehouse is not None:
                is_updated = self.save_warehouses_statuses(
                    order_id=order.get("entity_id"),
                    warehouse_code=warehouse,
                    status=ns_status,
                )
                if is_updated:
                    warehouse_status_comment = (
                        "Update Warehouse ({warehouse}) Status to {ns_status}".format(
                            warehouse=warehouse, ns_status=ns_status
                        )
                    )
                    self.mage2OrderConnector.insert_order_comment(
                        order_id=order.get("entity_id"),
    ...
    def transform_ns_order_status(self, transaction):
        ...
        if transaction["data"].get("status") in ["canceled", "Cancelled"]:
            ...
            if transaction["data"].get("fulfill_ship_status"):
                ...
                status = "{if_status}{suffix}".format(
                    if_status=if_status, suffix=suffix
                )
        return status
    ...
    def insert_offline_order_by_default_api(self, increment_id, transaction):
        ...
        customer_id = self.get_customer_id_by_company_no(
            transaction["data"].get("customer_id")
        )
        if customer_id is None:
            raise Exception(
                f"{tx_type_src_id}: Can not find customer by company NO in Magento"
            )
        ...
        for item in items:
            ...
            if product_id != 0:
                type_id = self.mage2Connector.get_product_type_id_by_sku(
                    item.get("sku")
                )
                product_name = self.mage2Connector.get_entity_attribute_value(
                    "catalog_product", product_id, "name", store_id=0
                )
                weight = self.mage2Connector.get_entity_attribute_value(
                    "catalog_product", product_id, "weight", store_id=0
                )
                if weight is None:
                    weight = 0.001
                ...
                total_qty_ordered = total_qty_ordered + float(
                    item.get("qty_ordered", 0)
                )
            else:
                raise Exception(
                    f"{tx_type_src_id}: Sku: '{item.get('sku')}' not in Magento"
                )
        ...
        posts = {
            "entity": {
                ...
                "items": [],
                "payment": {
                    "method": self.setting.get(
                        "offline_default_payment_method", "checkmo"
                    )
                },
                "extension_attributes": {
                    "amasty_order_attributes": [
                        {"attribute_code": "is_offline_order", "value": True}
                    ],
                            ...
                            "shipping": {
                                "address": {
                                    ...
                                    "lastname": (
                                        (
                                            " ".join(
                                                shipping_address.get("contact").split(
                                                    " "
                                                )[1:]
                                            )
                                            if len(
                                    ...
                                    "telephone": shipping_address.get(
                                        "telephone", "######"
                                    ),
                                },
                                "method": self.setting.get(
                                    "offline_default_shipping_method",
    ...
    def get_customer_id_by_company_no(self, company_no):
        try:
            (data_type, attribute_metadata) = (
                self.mage2Connector.get_attribute_metadata("company_no", "customer")
            )
    ...
    def get_customer_data(self, customer_id):
        self.mage2Connector.adaptor.mysql_cursor.execute(
            "SELECT * FROM customer_entity WHERE entity_id = %s;", [customer_id]
        )
        res = self.mage2Connector.adaptor.mysql_cursor.fetchone()
        return res
    ...
    def save_warehouses_statuses(self, order_id, warehouse_code, status):
        try:
            warehouses_statuses = {}
            (entity_id, origin_value) = self.get_order_attribute_value(
                order_id=order_id, attribute_code="warehouse_statuses"
            )
            if origin_value is not None:
                warehouses_statuses = json.loads(origin_value)
            warehouses_statuses.update({warehouse_code: status})
            warehouses_statuses_string = json.dumps(warehouses_statuses)
            if origin_value == warehouses_statuses_string:
                return False
            self.save_order_attribute_value(
                order_id=order_id,
                attribute_code="warehouse_statuses",
                value=warehouses_statuses_string,
                entity_id=entity_id,
            )
            return True
        except Exception as e:
    ...
    def get_order_attribute_value(self, order_id, attribute_code):
        (data_type, attribute_metadata) = self.mage2Connector.get_attribute_metadata(
            attribute_code, "amasty_checkout"
        )
        attribute_id = attribute_metadata["attribute_id"]
        attribute_value_table = "amasty_order_attribute_entity_{data_type}".format(
            data_type=data_type
        )
        sql = """
        ...
        self.mage2Connector.adaptor.mysql_cursor.execute(sql, [attribute_id, order_id])
        res = self.mage2Connector.adaptor.mysql_cursor.fetchone()
        value = None
        entity_id = None
        if res is not None:
    ...
    def save_order_attribute_value(
        self, order_id, attribute_code, value, entity_id=None
    ):
        try:
            (data_type, attribute_metadata) = (
                self.mage2Connector.get_attribute_metadata(
                    attribute_code, "amasty_checkout"
                )
            )
        except Exception as e:
        ...
        self.mage2Connector.adaptor.mysql_cursor.execute(
            sql, [attribute_id, entity_id, value, value]
        )
    ...
    def save_coa_files(self, order_id, coa_files=[]):
        ...
        try:
            ...
            self.mage2Connector.adaptor.mysql_cursor.execute(
                ...
                ON DUPLICATE KEY UPDATE `value`=%s;
                """,
                [attribute_id, coa_string, order_id, coa_string],
            )
            self.mage2Connector.adaptor.commit()
        except Exception as e:
